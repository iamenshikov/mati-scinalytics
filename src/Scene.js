...
export function Scene(props) {
  ...
  // Get the priced item
  const price = { КНОКСХУЛЬТ: 5999, Коврик: 433, Стул: 77, Стол: 255, СКАФТЕТ: 1699, ЛИВСВЕРК: 44 }[hovered] ?? 5999
  return (
    <>
      <group {...props}>
        ...
        <Select enabled={hovered === "Коврик"} onPointerOver={over("Коврик")} onPointerOut={() => debouncedHover(null)}>
          <mesh geometry={nodes.carpet.geometry} material={materials.carpet} />
        </Select>
        <Select enabled={hovered === "Стол"} onPointerOver={over("Стол")} onPointerOut={() => debouncedHover(null)}>
          <mesh geometry={nodes.table.geometry} material={materials.walls} material-envMap={env} material-envMapIntensity={0.5} />
        </Select>
        <Select enabled={hovered === "Стул"} onPointerOver={over("Стул")} onPointerOut={() => debouncedHover(null)}>
        ...
        <Select enabled={hovered === "ЛИВСВЕРК"} onPointerOver={over("ЛИВСВЕРК")} onPointerOut={() => debouncedHover(null)}>
          <mesh geometry={nodes.vase.geometry} material={materials.gray} material-envMap={env} />
        </Select>
        <Select enabled={hovered === "СКАФТЕТ"} onPointerOver={over("СКАФТЕТ")} onPointerOut={() => debouncedHover(null)}>
      ...
      <Text position={[1, 1.25, 0]} color="black" fontSize={0.15} font="Inter-Regular.woff" letterSpacing={-0.05}>
        {hovered ? hovered : "Кухонная мебель"}
      </Text>
